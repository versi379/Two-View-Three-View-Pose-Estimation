...
where the involved \acs{FM}s are
\begin{equation}
...
\begin{algorithm}[h]
		...
		\kwObjective{Given a set of point and line correspondences in three views, compute the \acs{TFT}.}
		\kwAlgorithm{
		\begin{enumerate}[label=(\roman*),leftmargin=*,rightmargin=1.5em]
        	\item From the set of point and line correspondences compute the set of equations of the form \( At = 0 \), where \( t \) is the 27-vector made up of the entries of the \acs{TFT}.
        	\item Solve these equations using the least-squares solution to constrained systems, in order to find an initial estimate of \( \mathbfcal{T}_i^{jk} \).
...
containing only 10 non-zero parameters, up to scale) where the tensor operation \( \otimes \) corresponds to the matrix operation on the slices \( \bm{\widetilde{T_i}} = V^\top (\sum_{m}U_{m,i}\bm{T_m}) W \). The scale can be fixed by imposing \( || \bm{\widetilde{\mathbfcal{T}}} || = 1 \).\\
